...
import arc.util.*;
import arc.util.io.*;
import classicMod.library.classCustom.*;
...
public class PayloadLaunchPad extends PayloadBlock{
    ...
    public class PayloadLaunchPadBuild extends PayloadBlockBuild<Payload>{
        ...
        public Cursor getCursor(){
            return !state.isCampaign() || net.client() ? Cursor.arrow : super.getCursor();
        }*/

        @Override
        public Cursor getCursor(){
            return !state.isCampaign() || net.client() ? SystemCursor.hand : SystemCursor.arrow;
        }

        //cannot be disabled
        @Override
        public float efficiency(){
            return  power != null && ((block.consumesPower) && !block.consPower.buffered) ? power.status : 1f;
            //return power != null && (block.consume.has(ConsumeType.power) && !block.consumes.getPower().buffered) ? power.status : 1f;
        }
