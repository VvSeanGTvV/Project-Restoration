...
public class JumpingUnitType extends UnitType {
    ...
    public void draw(Unit unit) {
        if(unit.controller() instanceof JumpingAI ai) {
            ...
            Draw.xscl = -Mathf.sign(unit.rotation < 90);
            var sine = Mathf.sin(ai.timing);
            Draw.z(Layer.groundUnit);
            if (sine < -0.85f) ai.timing = 2f;
            if (sine > 0f && !ai.stopMoving) {
                Draw.rect(region, unit.x, unit.y + 2 - sine * 2, (((float) region.width / 2) + sine * 5) * Draw.xscl, ((float) region.height / 2) - sine * 10);
                if(ai.hit) Draw.rect(ouch, unit.x, unit.y + 2 - sine * 2, (((float) ouch.width / 2) + sine * 5) * Draw.xscl, ((float) ouch.height / 2) - sine * 10);
            } else {
