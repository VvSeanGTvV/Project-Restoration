    ...
    public void draw(Unit unit) {
        if(unit.controller() instanceof JumpingAI ai) {
            ...
            if (sine > 0f && !ai.stopMoving) {
                Draw.rect(region, unit.x, unit.y + 2 + sine * 4, (((float) region.width / 2) + sine * 5) * Draw.xscl, ((float) region.height / 2) - sine * 10);
                if(ai.hit) Draw.rect(ouch, unit.x, unit.y + 2 + sine * 4, (((float) ouch.width / 2) + sine * 5) * Draw.xscl, ((float) ouch.height / 2) - sine * 10);
            } else {
