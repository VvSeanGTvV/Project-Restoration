    ...
    public void draw(Unit unit) {
        if(unit.controller() instanceof JumpingAI ai) {
            ...
            if (sine > 0f && !ai.stopMoving) {
                ...
                if(ai.hit) Draw.rect(ouch, unit.x, unit.y + 2 - sine * 2, ((float) region.width / 2) + sine * 5, ((float) region.height / 2) - sine * 10);
            } else {
                Draw.rect(region, unit.x, unit.y + 2, (float) region.width / 2, (float) region.height / 2);
                if(ai.hit) Draw.rect(ouch, unit.x, unit.y + 2, ((float) region.width / 2) + sine * 5, ((float) region.height / 2) - sine * 10);
            }
        }
