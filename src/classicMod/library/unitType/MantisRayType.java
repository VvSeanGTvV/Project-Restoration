...
public class MantisRayType extends UnitType {
    ...
    public void drawTail(Unit unit, MantisRayAI ai) {
        ...
        float lRot0 = ai.lastRot - unit.rotation;
        Tmp.v1.trns(unit.rotation + sine0 + lRot0 + AngleOffset[0] - 90, offsetX - ((lRot0 / 15f) + sine0 / 5f), ((TailBegin.height / 8f) + 6.6f + padding) * sclr);
        ...
        sclr = 1f;
        float lRot1 = ai.lastRotEnd - unit.rotation;
        Tmp.v1.trns(unit.rotation + sine0 + lRot1 + AngleOffset[1] - 90, offsetX - ((lRot1 / 15f) + sine0 / 5f) - (Mathf.sin(ai.timer) / 2f), ((TailMiddle.height / 4f) + 0.15f + padding) * sclr);
